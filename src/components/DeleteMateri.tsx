...
interface DeleteMateriProps {
  id: string;
  title: string;
  className?: string;
}

export default function DeleteMateri({ id, title, className }: DeleteMateriProps) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setStep(1)}
        className={cn(
          "h-10 w-10 flex items-center justify-center rounded-xl bg-secondary/50 text-muted-foreground hover:text-destructive border border-border/50 transition-all duration-300",
          className
        )}
        title={`Hapus ${title}`}
      >
            ...
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground font-medium px-4">
                  {step === 1 && `Konfirmasi penghapusan materi: "${title}".`}
                  {step === 2 && "Data yang telah dihapus tidak dapat dikembalikan lagi. Lanjutkan?"}
                  {step === 3 && "Materi akan menghilang selamanya dari database. Hati-hati!"}
                </p>
              </div>
